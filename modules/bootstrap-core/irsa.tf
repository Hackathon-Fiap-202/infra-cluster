...
resource "aws_iam_role" "external_secrets" {
  ...
  assume_role_policy = jsonencode({
    ...
    Statement = [{
      ...
      Condition = {
        StringEquals = {
          "${replace(var.cluster_oidc_provider_url, "https://", "")}:sub" = "system:serviceaccount:external-secrets:external-secrets-sa"
        }
      }
    }]
  })
...
resource "aws_iam_role" "ebs_csi" {
  ...
  assume_role_policy = jsonencode({
    ...
    Statement = [{
      ...
      Condition = {
        StringEquals = {
          "${replace(var.cluster_oidc_provider_url, "https://", "")}:sub" = "system:serviceaccount:kube-system:ebs-csi-controller-sa"
        }
      }
    }]
  })
